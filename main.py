...
_orig_dw = layers.DepthwiseConv2D.__init__
def _patch_dw(self, *a, **kw):
    kw.pop("groups", None)
    _orig_dw(self, *a, **kw)
layers.DepthwiseConv2D.__init__ = _patch_dw
...
class ViolenceDetector:
    ...
    def __init__(self, seq_model_path: str, input_size: int = 128, seq_len: int = 10):
        log.info(f"Dang khoi dong AI Engine: {seq_model_path}...")
        self.seq_model = load_model(seq_model_path)

        in_shape = self.seq_model.input_shape
        if isinstance(in_shape, list):
            in_shape = in_shape[0]

        if len(in_shape) == 4:
            self.is_end_to_end = True
            self.input_size = in_shape[1] if in_shape[1] is not None else input_size
            ...
            dummy_img = tf.zeros((1, self.input_size, self.input_size, 3), dtype=tf.float32)
            _ = self._fast_predict(dummy_img)
        else:
            self.is_end_to_end = False
        ...
        log.info(f"AI Engine san sang hoat dong! (End-to-End: {self.is_end_to_end}, Input: {self.input_size}x{self.input_size})")
    ...
    def _worker_infer(self, rgb_frame: np.ndarray):
        try:
            ...
            if self.is_end_to_end:
                preds = self._fast_predict(inp).numpy()
                if preds.ndim == 2 and preds.shape[1] == 1:
                    viol_prob = float(preds[0][0])
                elif preds.ndim == 2 and preds.shape[1] >= 2:
                    viol_prob = float(preds[0][1])
                else:
                    viol_prob = float(preds[0])
            else:
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                min_pair_dist = 999.0
                is_combat_motion = False
                for i_idx in range(n_persons):
                    for j_idx in range(i_idx + 1, n_persons):
                        b1 = person_boxes[i_idx]
                        b2 = person_boxes[j_idx]
                        avg_h_p = (b1.height + b2.height) / 2.0
                        avg_w_p = max(20.0, (b1.width + b2.width) / 2.0)
                        d_p = math.hypot(
                            b1.center[0] - b2.center[0],
                            b1.center[1] - b2.center[1]
                        ) / max(1.0, avg_h_p)
                        if d_p < min_pair_dist:
                            min_pair_dist = d_p
                        dg = abs(b1.y2 - b2.y2) / max(1.0, avg_h_p)
                        x_ov = max(0.0, min(b1.x2, b2.x2) - max(b1.x1, b2.x1))
                        ov_r = x_ov / avg_w_p
                        p_min_m = min(b1.motion_score, b2.motion_score)
                        p_max_m = max(b1.motion_score, b2.motion_score)
                        if d_p <= 1.10:
                            if ov_r >= 0.22 and p_max_m >= 2.6 and p_min_m >= 1.6 and (p_max_m + p_min_m >= 4.5):
                                is_combat_motion = True
                            elif (ov_r < 0.22 or x_ov == 0) and dg <= 0.45 and p_max_m >= 3.5 and p_min_m >= 1.9 and (p_max_m + p_min_m >= 5.6):
                                is_combat_motion = True
                ...
                elif n_fighting >= 2 and (conf >= 45.0 or is_violence or max_box_motion >= 3.5):
                    is_violence = True
                    lbl = "Violence"
                    conf = max(conf, 90.0)
                ...
                has_physical_combat = (
                    ...
                    (is_combat_motion and conf >= 70.0) or
                    (n_persons >= 2 and max_box_motion >= 4.2 and conf >= 72.0)
                )

                if (conf >= 70.0 or is_violence) and has_physical_combat:
                    is_violence = True
                    lbl = "Violence"
                    conf = max(conf, 88.0)
